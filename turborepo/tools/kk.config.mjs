...
export const config = {
  ...
  forbiddenPatterns: [
    ...
    { id: 'adr001-api-route', re: 'export\\s+(async\\s+)?function\\s+(GET|POST|PUT|PATCH|DELETE)\\b', appliesTo: 'apps/.*/app/api/.*route\\.(ts)$', msg: 'engineering_standards.md §2 + ADR-001: brak API routes dla logiki wewnętrznej. Webhooki zewnętrzne (Stripe/P24, kurier, Polar) to wyjątek — dopisz ścieżkę do allowIn.', allowIn: ['app/api/webhooks/', 'app/api/field/'] },
