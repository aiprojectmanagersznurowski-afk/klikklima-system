...
const ALLOWED_TRANSITIONS: Record<LeadStatus, LeadStatus[]> = {
  ...
  // FNL-ADVANCE-STATUS-CONTRACT-BOUND (Faza 1, Wariant B, decyzja człowieka 2026-09-07):
  // T10-T13 (wszystkie przejścia -> ROLLBACK_RESCHEDULING) USUNIĘTE z tej mapy.
  // advanceLeadStatus(leadId, 'ROLLBACK_RESCHEDULING') z dowolnego stanu źródłowego
  // trafia teraz w twardą odmowę K2 (kontrakt nie zna tego przejścia jako lokalnego
  // skrótu) — ten sam mechanizm, który blokuje AWAITING_AUDIT -> NEW_LEAD od Fali C.
  AWAITING_CREW_ASSIGNMENT: ["HARDWARE_IN_WAREHOUSE"],
  HARDWARE_IN_WAREHOUSE: ["HARDWARE_IN_TRANSIT", "AWAITING_INSTALLATION"],
  HARDWARE_IN_TRANSIT: ["AWAITING_INSTALLATION"],
  AWAITING_INSTALLATION: ["INSTALLATION_COMPLETED"],
