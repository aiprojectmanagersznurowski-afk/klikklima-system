...
import { ThemeSwitcher } from "@/app/(dashboard)/_components/header/theme-switcher";

export default function LoginScreen() {
  const [phase, setPhase] = useState<"splash" | "transitioning" | "login">(
    "splash",
  );
        ...
        >
          <div className="relative flex items-center justify-center transition-transform duration-500 hover:scale-105">
            <img
              src="/logo.png"
              alt="KlikKlima"
              className="h-28 sm:h-36 md:h-44 w-auto object-contain drop-shadow-[0_12px_28px_rgba(23,80,200,0.25)] dark:hidden"
            />
            <img
              src="/logo_dark.png"
              alt="KlikKlima"
              className="h-28 sm:h-36 md:h-44 w-auto object-contain drop-shadow-[0_12px_28px_rgba(56,182,232,0.25)] hidden dark:block"
            />
          </div>
        </div>
      )}
          ...
          <CardHeader className="text-center space-y-3 pb-6 pt-8">
            <div className="flex justify-center mb-1">
              <img
                src="/logo.png"
                alt="Klik Klima"
                className="h-[76px] w-auto object-contain drop-shadow-sm transition-transform hover:scale-105 dark:hidden"
              />
              <img
                src="/logo_dark.png"
                alt="Klik Klima"
                className="h-[76px] w-auto object-contain drop-shadow-sm transition-transform hover:scale-105 hidden dark:block"
              />
            </div>
            <div>
            ...
            {status === "idle" ? (
              <>
                ...
                >
                  <svg
                    viewBox="0 0 24 24"
                    className="size-5 bg-white rounded-full p-0.5 fill-current shrink-0"
                  >
                    <path
                ...
                <div className="bg-destructive/10 border border-destructive/20 text-destructive p-4 rounded-xl flex gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-xs opacity-90 leading-relaxed">
                      Twój adres e-mail nie znajduje się na liście uprawnionych
                      użytkowników. Skontaktuj się z administratorem systemu,
