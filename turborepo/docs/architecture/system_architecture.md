...
| :--- | :--- | :--- | :--- | :--- |
| **1. Warstwa Fundamentów (Core)** | **Turborepo** (zarządzanie monorepo), TypeScript | **Supabase** (PostgreSQL, Auth, Storage) | **Supabase Cloud** | Supabase to gotowy backend. Eliminuje konieczność pisania własnego serwera API od zera. Daje potężną relacyjną bazę i gotowe systemy logowania z gigantycznym darmowym planem na start. |
| **2. Kliencka B2C (Formularz Triage)** | **Next.js (React)** + Tailwind CSS + React Hook Form | Łączy się bezpośrednio z Supabase / API | **Vercel** | Next.js jest natywną technologią Vercel. Daje świetne SEO (Server-Side Rendering) i błyskawiczne ładowanie. Wdrożenie na produkcję to dosłownie podłączenie repozytorium GitHub do panelu Vercel. |
