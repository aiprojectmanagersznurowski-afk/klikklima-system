...
flowchart TB
    subgraph Monorepo["📦 Monorepo (Turborepo)"]
        ...
        subgraph B2B["💻 3. Panel Administracyjny B2B (Next.js App Router)"]
...
| **3. Panel B2B (Dyspozytor)** | **Next.js (App Router)** + Tailwind CSS + shadcn/ui | **Prisma na serwerze** (Server Actions, z pominięciem RLS); `supabase-js` wyłącznie do sesji i Auth | **Vercel** | Panel jest zamknięty i nie potrzebuje SEO, ale potrzebuje czegoś innego: dostępu do danych bez wystawiania zapytań do przeglądarki. App Router realizuje odczyt w Server Components, a mutacje w Server Actions, więc klucze i logika uprawnień nie opuszczają serwera. To także jedyny wariant zgodny z `engineering_standards.md`, który zakazuje API routes dla logiki wewnętrznej — Vite jako SPA wymagałby dokładnie tej warstwy, którą standard odrzuca. Komponenty `shadcn/ui` pozostają bez zmian. |
